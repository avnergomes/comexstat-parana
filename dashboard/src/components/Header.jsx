import { Ship, Package, Globe, Calendar } from 'lucide-react';
...
export default function Header({ metadata }) {
  const stats = metadata ? [
    ...
    { icon: Package, label: 'Produtos', value: formatNumber(metadata.produtosExp) },
    { icon: Globe, label: 'Paises', value: formatNumber(metadata.paisesDestino) },
    { icon: Ship, label: 'Registros', value: formatNumber(metadata.totalExportacoes + metadata.totalImportacoes) },
  ] : [];
